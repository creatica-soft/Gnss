...
template <class T, class D> void Gnss<T, D>::cfgTp() {
	...
	if (f.isFreq) { strcat(flags, String(F("|isFreq")).c_str()); sprintf(fp, String(F("Hz")).c_str()); }
	else strcat(flags, String(F("|isPeriod")).c_str());
	if (f.isLength) { strcat(flags, String(F("|isLength")).c_str()); sprintf(lc, String(F("us")).c_str()); }
	else strcat(flags, String(F("|Cycles")).c_str());
...
template <class T> void Gnss<T, null>::nmeaTxt() {
}

template <class T, class D> void Gnss<T, D>::nmeaTxt() {
	GNTXT data;
	char t[8] = "";
	getGNTXT(&data);
	switch (data.msgType) {
	...
	char s[255];
	sprintf(s, String(F("GNTXT: message %u out of %u \"%s! %s\"")).c_str(), data.msgNum, data.numMsg, t, data.text.c_str());
	debug.println(s);
}

template <class T> void Gnss<T, null>::nmeaGbs() {
}

template <class T, class D> void Gnss<T, D>::nmeaGbs() {
	GBS data;
	getGbs(&data);
	tm tm_time;
	time_t t = mk_gmtime(gps2tm(&(data.dateTime), &tm_time));
	char s[255];
	sprintf(s, String(F("GBS: %s Lat err %.1fm, Lon err %.1fm, Alt err %.1fm, failed svId %u, bias %.1fm, stddev %.1fm, sysId %u, sigId %u")).c_str(), asctime(gmtime(&t)), (double)(data.errLat), (double)(data.errLon), (double)(data.errAlt), data.svId, (double)(data.bias), (double)(data.stddev), data.systemId, data.signalId);
	debug.println(s);
}

template <class T> void Gnss<T, null>::nmeaDtm() {
}

template <class T, class D> void Gnss<T, D>::nmeaDtm() {
	DTM data;
	getDtm(&data);
	char s[128];
	sprintf(s, String(F("DTM: datum %s differs from refDatum %s by %.1f\'%c %.1f\'%c %.1fm")).c_str(), data.datum.c_str(), data.refDatum.c_str(), (double)(data.lat), data.NS, (double)(data.lon), data.EW, (double)(data.alt));
	debug.println(s);
}

template <class T> void Gnss<T, null>::nmeaGrs() {
}

template <class T, class D> void Gnss<T, D>::nmeaGrs() {
	GRS data;
	getGrs(&data);
	tm tm_time;
	...
	char sss[128];
	sprintf(sss, String(F("GRS: %s %s sysId %u, sigId %u")).c_str(), asctime(gmtime(&t)), ss, data.systemId, data.signalId);
	debug.println(sss);
}

template <class T> void Gnss<T, null>::nmeaGpq(const char* talkerId, const char* msgId) {
	uint8_t x = 0;
	char s[32];
	memset(s, 0, 32);
...
template <class T, class D> void Gnss<T, D>::nmeaGpq(const char* talkerId, const char* msgId) {
	uint8_t x = 0;
	char s[32];
	memset(s, 0, 32);
...
template <class T> void Gnss<T, null>::nmeaGnq(const char* talkerId, const char* msgId) {
	uint8_t x = 0;
	char s[32];
	memset(s, 0, 32);
...
template <class T, class D> void Gnss<T, D>::nmeaGnq(const char* talkerId, const char* msgId) {
	uint8_t x = 0;
	char s[32];
	memset(s, 0, 32);
...
template <class T> void Gnss<T, null>::nmeaGlq(const char* talkerId, const char* msgId) {
	uint8_t x = 0;
	char s[32];
	memset(s, 0, 32);
...
template <class T, class D> void Gnss<T, D>::nmeaGlq(const char* talkerId, const char* msgId) {
	uint8_t x = 0;
	char s[32];
	memset(s, 0, 32);
...
template <class T> void Gnss<T, null>::nmeaGbq(const char* talkerId, const char* msgId) {
	uint8_t x = 0;
	char s[32];
	memset(s, 0, 32);
...
template <class T, class D> void Gnss<T, D>::nmeaGbq(const char* talkerId, const char* msgId) {
	uint8_t x = 0;
	char s[32];
	memset(s, 0, 32);
...
template <class T> bool Gnss<T, null>::config(ConfMask mask, ConfigType type, uint32_t timeout) {
	ConfigAllDevs conf;
	switch (type)
	{
...
template <class T, class D> bool Gnss<T, D>::config(ConfMask mask, ConfigType type, uint32_t timeout) {
	ConfigAllDevs conf;
	switch (type)
	{
		case DEFAULT_CONFIG: conf.clearMask = mask; memset(&(conf.saveMask), 0, sizeof(ConfMask)); memset(&(conf.loadMask), 0, sizeof(ConfMask)); break;
...
template <class T> bool Gnss<T, null>::config(ConfMask mask, ConfigType type, Device dev, uint32_t timeout) {
	ConfigDev conf;
	switch (type)
	{
		case DEFAULT_CONFIG: conf.clearMask = mask; memset(&(conf.saveMask), 0, sizeof(ConfMask)); memset(&(conf.loadMask), 0, sizeof(ConfMask)); break;
...
template <class T, class D> bool Gnss<T, D>::config(ConfMask mask, ConfigType type, Device dev, uint32_t timeout) {
	ConfigDev conf;
	switch (type)
	{
		case DEFAULT_CONFIG: conf.clearMask = mask; memset(&(conf.saveMask), 0, sizeof(ConfMask)); memset(&(conf.loadMask), 0, sizeof(ConfMask)); break;
...
template <class T> bool Gnss<T, null>::defaultConfig(bool ioPort, bool msgConf, bool infMsg, bool navConf, bool rxmConf, bool senConf, bool rinvConf, bool antConf, bool logConf, bool ftsConf, uint32_t timeout) {
	ConfMask conf;
...
template <class T, class D> bool Gnss<T, D>::defaultConfig(bool ioPort, bool msgConf, bool infMsg, bool navConf, bool rxmConf, bool senConf, bool rinvConf, bool antConf, bool logConf, bool ftsConf, uint32_t timeout) {
	ConfMask conf;
	conf.ioPort = ioPort ? 1 : 0;
	conf.msgConf = msgConf ? 1 : 0;
	conf.infMsg = infMsg ? 1 : 0;
...
template <class T> bool Gnss<T, null>::saveConfig(bool ioPort, bool msgConf, bool infMsg, bool navConf, bool rxmConf, bool senConf, bool rinvConf, bool antConf, bool logConf, bool ftsConf, uint32_t timeout) {
	ConfMask conf;
	conf.ioPort = ioPort ? 1 : 0;
	conf.msgConf = msgConf ? 1 : 0;
	conf.infMsg = infMsg ? 1 : 0;
...
template <class T, class D> bool Gnss<T, D>::saveConfig(bool ioPort, bool msgConf, bool infMsg, bool navConf, bool rxmConf, bool senConf, bool rinvConf, bool antConf, bool logConf, bool ftsConf, uint32_t timeout) {
	ConfMask conf;
	conf.ioPort = ioPort ? 1 : 0;
	conf.msgConf = msgConf ? 1 : 0;
	conf.infMsg = infMsg ? 1 : 0;
...
template <class T> bool Gnss<T, null>::loadConfig(bool ioPort, bool msgConf, bool infMsg, bool navConf, bool rxmConf, bool senConf, bool rinvConf, bool antConf, bool logConf, bool ftsConf, uint32_t timeout) {
	ConfMask conf;
	conf.ioPort = ioPort ? 1 : 0;
	conf.msgConf = msgConf ? 1 : 0;
	conf.infMsg = infMsg ? 1 : 0;
...
template <class T, class D> bool Gnss<T, D>::loadConfig(bool ioPort, bool msgConf, bool infMsg, bool navConf, bool rxmConf, bool senConf, bool rinvConf, bool antConf, bool logConf, bool ftsConf, uint32_t timeout) {
	ConfMask conf;
	conf.ioPort = ioPort ? 1 : 0;
	conf.msgConf = msgConf ? 1 : 0;
	conf.infMsg = infMsg ? 1 : 0;
...
template <class T> void Gnss<T, null>::cfgRst(StartType type, ResetMode mode) {
	CfgRst rset;
...
template <class T, class D> void Gnss<T, D>::cfgRst(StartType type, ResetMode mode) {
	CfgRst rset;
	rset.navBbrMask = type.mask;
	rset.resetMode = mode;
	poll(UBX_CFG, CFG_RST, sizeof(CfgRst), (uint8_t*)& rset);
...
template <class T> void Gnss<T, null>::stopGnss(StartTypes startType) {
	StartType type;
	type.start = startType;
	cfgRst(type, GNSS_STOP);
}

template <class T, class D> void Gnss<T, D>::stopGnss(StartTypes startType) {
	StartType type;
	type.start = startType;
	cfgRst(type, GNSS_STOP);
}

template <class T> void Gnss<T, null>::startGnss(StartTypes startType) {
	StartType type;
	type.start = startType;
	cfgRst(type, GNSS_START);
}

template <class T, class D> void Gnss<T, D>::startGnss(StartTypes startType) {
	StartType type;
	type.start = startType;
	cfgRst(type, GNSS_START);
}

template <class T> void Gnss<T, null>::resetGnss(StartTypes startType) {
	StartType type;
	type.start = startType;
	cfgRst(type, SOFTWARE_RESET_GNSS_ONLY);
}

template <class T, class D> void Gnss<T, D>::resetGnss(StartTypes startType) {
	StartType type;
	type.start = startType;
	cfgRst(type, SOFTWARE_RESET_GNSS_ONLY);
}

template <class T> void Gnss<T, null>::reset(bool soft, bool afterShutdown, StartTypes startType) {
	StartType type;
	type.start = startType;
...
template <class T, class D> void Gnss<T, D>::reset(bool soft, bool afterShutdown, StartTypes startType) {
	StartType type;
	type.start = startType;
	if (soft) cfgRst(type, SOFTWARE_RESET);
	else {
...
template <class T> void Gnss<T, null>::getGNTXT(GNTXT* data) {
	String txt[5];
	...
	for (uint8_t i = 1; i < 5; i++) {
		switch (i) {
		case 1: data->numMsg = txt[i].toInt(); break;
		case 2: data->msgNum = txt[i].toInt(); break;
		case 3: data->msgType = txt[i].toInt(); break;
		case 4: data->text = txt[i]; break;
		}
	}
}

template <class T, class D> void Gnss<T, D>::getGNTXT(GNTXT* data) {
	String txt[5];
	data->text = ""; data->msgNum = 0; data->numMsg = 0; data->msgType = 0;
...
template <class T> void Gnss<T, null>::getGbs(GBS* data) {
	String gbs[11];
	memset(&(data->dateTime), 0, sizeof(DateTime));
	...
	for (uint8_t i = 1; i < 11; i++) {
		...
		case 2: data->errLat = gbs[i].toFloat(); break;
		case 3: data->errLon = gbs[i].toFloat(); break;
		case 4: data->errAlt = gbs[i].toFloat(); break;
		case 5: data->svId = gbs[i].toInt(); break;
		case 6: data->prob = gbs[i].toFloat(); break;
		case 7: data->bias = gbs[i].toFloat(); break;
		case 8: data->stddev = gbs[i].toFloat(); break;
		case 9: data->systemId = gbs[i].toInt(); break;
		case 10: data->signalId = gbs[i].toInt(); break;
		}
	}
	nmeaToUtc(&(data->dateTime), nmeaDate, time);
}
template <class T, class D> void Gnss<T, D>::getGbs(GBS* data) {
	String gbs[11];
	memset(&(data->dateTime), 0, sizeof(DateTime));
	data->errLat = 0; data->errLon = 0; data->bias = 0; data->errAlt = 0;
	data->signalId = 0; data->stddev = 0; data->svId = 0; data->systemId = 0; data->prob = 0;
...
template <class T> void Gnss<T, null>::getGrs(GRS* data) {
	String grs[17];
	memset(&(data->dateTime), 0, sizeof(DateTime));
	...
	for (uint8_t i = 1; i < 17; i++) {
		if (grs[i] != "")
			...
			case 2: data->mode = grs[i].toInt(); break;
			case 3: data->residual[0] = grs[i].toFloat(); break;
			case 4: data->residual[1] = grs[i].toFloat(); break;
			case 5: data->residual[2] = grs[i].toFloat(); break;
			case 6: data->residual[3] = grs[i].toFloat(); break;
			case 7: data->residual[4] = grs[i].toFloat(); break;
			case 8: data->residual[5] = grs[i].toFloat(); break;
			case 9: data->residual[6] = grs[i].toFloat(); break;
			case 10: data->residual[7] = grs[i].toFloat(); break;
			case 11: data->residual[8] = grs[i].toFloat(); break;
			case 12: data->residual[9] = grs[i].toFloat(); break;
			case 13: data->residual[10] = grs[i].toFloat(); break;
			case 14: data->residual[11] = grs[i].toFloat(); break;
			case 15: data->systemId = grs[i].toInt(); break;
			case 16: data->signalId = grs[i].toInt(); break;
			}
	}
	nmeaToUtc(&(data->dateTime), nmeaDate, time);
}

template <class T, class D> void Gnss<T, D>::getGrs(GRS* data) {
	String grs[17];
	memset(&(data->dateTime), 0, sizeof(DateTime));
	data->signalId = 0; data->systemId = 0;
	for (uint8_t i = 0; i < 12; i++) data->residual[i] = 0;
...
template <class T> void Gnss <T, null>::getDtm(DTM* data) {
	String dtm[9];
	...
	for (uint8_t i = 1; i < 9; i++) {
		...
		case 2: break;
		case 3: data->lat = dtm[i].toFloat(); break;
		case 4: data->NS = dtm[i].charAt(0); break;
		case 5: data->lon = dtm[i].toFloat(); break;
		case 6: data->EW = dtm[i].charAt(0); break;
		case 7: data->alt = dtm[i].toFloat(); break;
		case 8: data->refDatum = dtm[i]; break;
		}
	}
}

template <class T, class D> void Gnss <T, D>::getDtm(DTM* data) {
	String dtm[9];
	data->lat = 0; data->lon = 0; data->alt = 0; data->subDatum = ""; data->datum = ""; data->refDatum = "";
